...
def benchmark(
    ...
    device = f"cuda:{gpu:d}" if gpu is not None else "cpu"
    ...
    for i, batch_size in enumerate(batch_sizes):
        for j, p in enumerate(probs):
            print(f"batch size: {batch_size}\tp: {p:.02f}")
...
batch_size = 7
gpu = 3 if torch.cuda.is_available() else None
...
n_steps = sample_randint(-5, 6, (batch_size,))
print(n_steps)
    ...
    for lazy in [True, False]:
        for m in module_list:
            try:
                ...
                traceback.print_exc()
                continue
            arr = benchmark(module, gpu=gpu, timeits=3)
        ...
        arr = benchmark(
            ...
            BatchRandomPitchShift(-4, 5, 16000, n_fft=256, hop_length=64, win_length=256, p=0.8, return_masks=True, lazy=lazy),
            input_shape=(16000,),
            gpu=gpu
        )
        np.save(f"time/{'lazy' if lazy else 'normal'}/BatchRandomPitchShift_{device}", arr)
        print(f"    mean: {arr.mean():.3f}"
              f"    std: {arr.std():.3f}"
              f"    min: {arr.min():.3f}"
              f"    max: {arr.max():.3f}"
